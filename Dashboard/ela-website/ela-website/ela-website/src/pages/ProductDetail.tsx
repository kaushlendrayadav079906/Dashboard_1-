...
const ProductDetail = () => {
  ...
  // Reset per-product state whenever the route id changes (navigating between products)
  useEffect(() => {
    setSelectedColor("");
    setSelectedSize("");
  ...
  const currentItemCode = product?.itemCode || product?.item_code || undefined;
  ...
  const handleAddToCart = () => {
    // Only require size selection when the product actually has size options
    const effectiveSize = selectedSize || (product.sizes.length === 0 ? product.name : "");
    if (!effectiveSize) {
